...
class FeaSet(torch_utils.data.Dataset): 
    def __init__(self, *args, **kwargs):
        super(FeaSet, self).__init__()
        self.text_list = kwargs.get("text_list", args[0]) # (N, D, T)
        self.audio_list = kwargs.get("audio_list", args[1]) # (N, D, T)
    ...
    def __getitem__(self, idx):
        ...
        cur_audio = self.audio_list[idx]
        
        result = (cur_text, cur_audio)
        return result

class WhisperSet(torch_utils.data.Dataset): 
    def __init__(self, *args, **kwargs):
        super(WhisperSet, self).__init__()
        self.whisper_list = kwargs.get("whisper_list", args[0]) # (N, D, T)
            
    def __len__(self):
        return len(self.whisper_list)

    def __getitem__(self, idx):
        cur_whisper = self.whisper_list[idx]
